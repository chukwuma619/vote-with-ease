...
import { creatElection } from "@/actions/election";
          ...
          <div className="space-y-2">
            ...
            <div className="relative shadow-sm">
              <input
                ...
                min={now}
                id="start_date"
                className="block accent-green-600 rounded-lg w-full border-0 px-4 py-3 text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-green-600 sm:text-sm sm:leading-6 focus-visible:outline-none"
              />
            </div>
            <div aria-live="polite" aria-atomic="true" className="space-y-2">
              {errorMessage?.errors?.start_date &&
                errorMessage.errors.start_date.map((msg, index) => {
                  return (
                    <p key={index} className="text-red-500 text-sm">
                      {msg}
                    </p>
          ...
          <div className="space-y-2">
            ...
            <div className="relative shadow-sm">
              <input
                ...
                name="end_date"
                id="end_date"
                className="block rounded-lg w-full border-0 px-4 py-3 text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-green-600 sm:text-sm sm:leading-6 focus-visible:outline-none"
              />
            </div>
            <div aria-live="polite" aria-atomic="true" className="space-y-2">
              {errorMessage?.errors?.end_date &&
                errorMessage.errors.end_date.map((msg, index) => {
                  return (
                    <p key={index} className="text-red-500 text-sm">
                      {msg}
                    </p>
