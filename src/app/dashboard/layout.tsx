      ...
      <header className="shadow py-2 lg:hidden">
        <nav className="flex justify-between items-center container mx-auto px-4">
          <Link href={"/"} className="font-semibold text-2xl">
            Vote<span className="text-green-700">Ease</span>
          </Link>
      ...
      <div className="container mx-auto">
                ...
                {SidebarData.map((nav, index) => {
                  return (
                    <li key={index}>
                      <Link
                        href={nav.href}
                        className={`text-sm font-medium rounded-md p-2 flex hover:bg-gray-100 ${
        ...
        <main className="lg:ml-72 p-4 lg:p-10 bg-gray-50  min-h-svh relative">
          <button
            ...
            aria-label="Go back"
            className="group mb-4 md:ml-6 flex h-10 w-10 items-center justify-center rounded-full bg-white shadow-md shadow-gary-800/5 ring-1 ring-gray-900/5 transition"
          >
