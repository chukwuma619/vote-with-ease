...
import { updateVoter } from "@/actions/voter";

export default function EditVoterForm({
  data,
}: {
  data: VoterType | undefined
}) {
  const initialState = { message: undefined, errors: {} };
  const updateVoterWithCode = updateVoter.bind(
    null,
    data?.election_unique_code!, data?.id!
  );
